...
function loadPdcaStatus(projectDir) {
  try {
    return require('../pdca/status').loadPdcaStatus(projectDir);
  } catch {
...
function savePdcaStatus(projectDir, status) {
  try {
    require('../pdca/status').savePdcaStatus(status, projectDir);
  } catch {
    // Fallback: write to canonical location
    const statusPath = path.join(projectDir, '.bkit', 'state', 'pdca-status.json');

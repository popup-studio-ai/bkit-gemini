 ...
 * Manages skill activation, agent delegation, and template resolution for Gemini CLI.
 *
...
function getPluginRoot() {
  ...
  if (process.env.BKIT_PLUGIN_ROOT) {
    _pluginRoot = process.env.BKIT_PLUGIN_ROOT;
    return _pluginRoot;
  }
...
function applyDefaults(yaml, skillPath) {
  ...
  return {
    ...
    classification: yaml.classification || null,
...
/**
 * Get skills filtered by Skills 2.0 classification.
 * Classification types: W (Workflow), C (Capability), H (Hybrid)
 *
 * @param {string} classification - One of: 'W', 'C', 'H'
 * @returns {Array<{ name: string, description: string, classification: string }>}
 */
function getSkillsByClassification(classification) {
  if (!classification) return listSkills();
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const metadata = parseSkillFrontmatter(skillMdPath);
      if (metadata.classification && metadata.classification.toUpperCase() === cls) {
        results.push({
          name: metadata.name || entry.name,
          description: (metadata.description || '').split('\n')[0].trim(),

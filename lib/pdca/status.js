...
function getPdcaStatusPath(projectDir) {
  ...
  const bkitPath = path.join(dir, '.bkit', 'state', 'pdca-status.json');
  const rootPath = path.join(dir, '.pdca-status.json');
  const legacyPath = path.join(dir, 'docs', '.pdca-status.json');

  // Priority: .bkit/state/ → root → docs/ (legacy)
  if (fs.existsSync(bkitPath)) return bkitPath;
  if (fs.existsSync(rootPath)) return rootPath;
  if (fs.existsSync(legacyPath)) return legacyPath;

  // Default: new canonical location
  return bkitPath;
}
...
function loadPdcaStatus(projectDir) {
  ...
  if (fs.existsSync(bkitPath)) {
    content = fs.readFileSync(bkitPath, 'utf-8');
    sourcePath = bkitPath;
  } else if (fs.existsSync(rootPath)) {
    content = fs.readFileSync(rootPath, 'utf-8');
    sourcePath = rootPath;

    // Auto-migrate root → .bkit/state/
    try {
      const stateDir = path.dirname(bkitPath);
      if (!fs.existsSync(stateDir)) fs.mkdirSync(stateDir, { recursive: true });
      fs.writeFileSync(bkitPath, content);
    } catch (e) { /* ignore */ }
  } else if (fs.existsSync(legacyPath)) {
    content = fs.readFileSync(legacyPath, 'utf-8');
    sourcePath = legacyPath;

    // Auto-migrate docs/ → .bkit/state/
    try {
      const stateDir = path.dirname(bkitPath);
      if (!fs.existsSync(stateDir)) fs.mkdirSync(stateDir, { recursive: true });
      fs.writeFileSync(bkitPath, content);

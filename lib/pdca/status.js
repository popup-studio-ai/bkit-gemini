...
function loadPdcaStatus(projectDir) {
  ...
  // Robust mapping for features -> activeFeatures
  if (status.features && !status.activeFeatures) {
    status.activeFeatures = status.features;
  }
  
  if (status.version !== '2.0') status = migrateStatusToV2(status);

...
function loadConfig(forceRefresh = false) {
  ...
  // Try loading from project directory (L3 Project)
  try {
    const { getAdapter } = require('../adapters');
    const adapter = getAdapter();
    const projectDir = adapter.getProjectDir();
    const projectConfigPath = path.join(projectDir, 'bkit.config.json');

    if (fs.existsSync(projectConfigPath)) {
      const projectConfig = safeJsonParse(fs.readFileSync(projectConfigPath, 'utf-8'), {});
      
      // Deep merge (simple version for config)
      if (projectConfig.pdca) {
        config.pdca = { ...config.pdca, ...projectConfig.pdca };
      }
      if (projectConfig.permissions) {
        config.permissions = { ...config.permissions, ...projectConfig.permissions };
      }
    }
  } catch (e) {

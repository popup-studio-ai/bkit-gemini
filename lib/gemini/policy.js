...
/**
 * Build a commandRegex pattern that matches both bare command and full-path variants.
 * Handles v0.35.0+ normalizeCommandName which preserves full paths like /usr/bin/rm.
 *
 * IMPORTANT: This regex is used by Gemini CLI's buildArgsPatterns() which prepends
 * `"command":"` before the regex. So our pattern is matched starting right after
 * the opening quote of the command value in the JSON-serialized args.
 *
 * Examples:
 *   'rm -rf' -> '([^"]*\\/)?rm -rf' (matches 'rm -rf ...', '/usr/bin/rm -rf ...')
 *   'rm'     -> '([^"]*\\/)?rm([ "])'  (matches 'rm', '/bin/rm', 'rm foo', but not 'rmdir')
 *   'git push --force' -> '([^"]*\\/)?git push --force'
 *
 * @param {string} prefix - The original commandPrefix value (e.g., 'rm -rf', 'rm', 'git push --force')
 * @returns {string} A regex pattern for commandRegex field in TOML
 */
function buildPathAwareCommandRegex(prefix) {
  ...
  // Optional path prefix: matches zero or more non-quote chars ending with /
  // e.g., matches "" (bare cmd), "/usr/bin/", "/bin/"
  const pathPrefix = '([^"]*\\\\/)?';

  if (args) {
    const escapedArgs = args.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    // command with args: match optional path prefix + command + space + args
    return `${pathPrefix}${escapedCmd} ${escapedArgs}`;
  }
  // bare command without args: match optional path prefix + command + (space or end-quote)
  // The end-quote or space prevents 'rm' from matching 'rmdir'
  return `${pathPrefix}${escapedCmd}([ "])`;
}
...
/**
 * Emit TOML lines for a command matching rule.
 * For CLI >= 0.35.0, uses commandRegex to handle full-path command names.
 * For CLI < 0.35.0, uses commandPrefix (legacy behavior).
 *
 * @param {string[]} lines - Array to push TOML lines into
 * @param {string} commandPrefix - The command prefix value
 * @param {boolean} useRegex - Whether to use commandRegex (v0.35.0+)
 */
function emitCommandMatchToml(lines, commandPrefix, useRegex) {
  if (useRegex) {
    const regex = buildPathAwareCommandRegex(commandPrefix);
...
function convertToToml(permissions) {
  ...
  if (useRegex) {
    lines.push('# NOTE: Uses commandRegex for v0.35.0+ full-path command safety');
    lines.push('');
  }
  ...
  if (denyRules.length > 0) {
    ...
    for (const rule of denyRules) {
      ...
      if (rule.commandPrefix) {
        emitCommandMatchToml(lines, rule.commandPrefix, useRegex);
      }
...
function generateExtensionPolicy(extensionRoot) {
  ...
  if (useRegex) {
    extLines.push('# NOTE: Uses commandRegex for v0.35.0+ full-path command safety');
    extLines.push('');
  }
  ...
  for (const rule of extRules) {
    ...
    emitCommandMatchToml(extLines, rule.commandPrefix, useRegex);
